      ...
      <div class="hidden lg:flex flex-col justify-between w-1/2 bg-[#0d1225] border-r border-[#1e2d45] p-12 relative overflow-hidden">
        <div class="absolute inset-0 pointer-events-none">
          @for (i of grid8; track i) {
            <div
              class="absolute top-0 bottom-0 border-r border-[#1e2d4520]"
              [style.left]="((i + 1) * 12.5) + '%'"
            ></div>
          }
          @for (i of grid8; track i) {
            <div
              class="absolute left-0 right-0 border-b border-[#1e2d4520]"
              [style.top]="((i + 1) * 12.5) + '%'"
            ></div>
          }
        </div>
        ...
        <p class="relative text-[10px] text-[#64748b]">
          © 2026 ApexTrade Securities LLC. All rights reserved.
        </p>
      </div>
        ...
        <div class="w-full max-w-sm">
          @if (mode !== 'admin') {
            ...
            <form (ngSubmit)="handle()" #authForm="ngForm" class="space-y-4">
              @if (mode === 'signup') {
                <div>
                  <label class="block text-xs text-[#64748b] mb-1.5 font-medium tracking-wide">
                    Full Name
                  </label>
                  <input
                    type="text"
              ...
              @if (error) {
                <p class="text-xs text-[#ef4444] bg-[#ef444418] px-3 py-2 rounded border border-[#ef444430]">
                  {{ error }}
                </p>
              }
            ...
            @if (mode === 'signin') {
              <div class="mt-4 text-center">
                <a href="#" class="text-xs text-[#64748b] hover:text-[#94a3b8] transition-colors">
                  Forgot password?
                </a>
              </div>
            }
          ...
          @if (mode === 'admin') {
            <div class="admin-auth-panel">
              ...
              <div class="mb-8">
                <h2 class="text-2xl font-semibold text-[#e2e8f0]">Admin sign in</h2>
                <p class="admin-auth-subtitle">
                  Sign in to access internal reports and audit tools.
                </p>
              </div>

              <form (ngSubmit)="handle()" #adminForm="ngForm" class="space-y-4">
                <div>
                  <label class="block text-xs text-[#64748b] mb-1.5 font-medium tracking-wide">
                    Admin email
                  </label>
...
export class AuthComponent {
  mode: 'signin' | 'signup' | 'admin' = 'signin';
  name = '';
  email = '';
  password = '';
  confirm = '';
  ...
  setMode(mode: 'signin' | 'signup' | 'admin'): void {
    this.mode = mode;
    this.error = '';
    this.resetForm();
  }

  tabClass(mode: 'signin' | 'signup'): string {
  ...
  handle(): void {
    ...
    const role: UserRole = this.mode === 'admin' ? 'admin' : 'client';
